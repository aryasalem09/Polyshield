...
export function assertPathArgsInScope(args, root, options = {}) {
  ...
  const walk = (key, value, depth) => {
    if (depth > 8) throw new Error("Polyshield sandbox: argument tree too deep, refusing to validate");
...
export function remapArgs(args, fromRoot, toRoot) {
  const walk = (key, value, depth) => {
    if (depth > 8) throw new Error("Polyshield sandbox: argument tree too deep, refusing to remap");
